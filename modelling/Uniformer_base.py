...
model_path = {
    ...
    'MaxFlowUFOneView_MultiVSL200_center': os.path.join(model_path, 'MaskUFOneView/MaskUFOneView MultiVLS200 center MaxFlow mask ratio 0.4/checkpoint_epoch_300.pth'),
    'MaxFlowUFOneView_MultiVSL200_left': os.path.join(model_path, 'MaskUFOneView/MaskUFOneView MultiVLS200 left MaxFlow mask ratio 0.4/checkpoint_epoch_300.pth'),
    'MaxFlowUFOneView_MultiVSL200_right': os.path.join(model_path, 'MaskUFOneView/MaskUFOneView MultiVLS200 right MaxFlow mask ratio 0.4/checkpoint_epoch_300.pth'),
    
    'MaxFlowUFOneView_MMAuslan_center': os.path.join(model_path, 'MaskUFOneView/MaskUFOneView MMAuslan center MaxFlow mask ratio 0.4/checkpoint_epoch_300.pth'),
    'MaxFlowUFOneView_MMAuslan_left': os.path.join(model_path, 'MaskUFOneView/MaskUFOneView MMAuslan left MaxFlow mask ratio 0.4/checkpoint_epoch_300.pth'),
    'MaxFlowUFOneView_MMAuslan_right': os.path.join(model_path, 'MaskUFOneView/MaskUFOneView MMAuslan right MaxFlow mask ratio 0.4/checkpoint_epoch_300.pth'),

    'FromMaxFlowMask_MultiVSL200_center': os.path.join(model_path, 'UFOneView/UFOneView MultiVLS200 from MaxFlow MaskUFOneView MultiVLS200 center/best_checkpoints_loss.pth'),
    'FromMaxFlowMask_MultiVSL200_left': os.path.join(model_path, 'UFOneView/UFOneView MultiVLS200 from MaxFlow MaskUFOneView MultiVLS200 left/best_checkpoints_loss.pth'),
    'FromMaxFlowMask_MultiVSL200_right': os.path.join(model_path, 'UFOneView/UFOneView MultiVLS200 from MaxFlow MaskUFOneView MultiVLS200 right/best_checkpoints_loss.pth'),

    'FromMaxFlowMask_MMAuslan_center': os.path.join(model_path, 'UFOneView/UFOneView MMAuslan from MaxFlow MaskUFOneView MMAuslan center/best_checkpoints_loss.pth'),
    'FromMaxFlowMask_MMAuslan_left': os.path.join(model_path, 'UFOneView/UFOneView MMAuslan from MaxFlow MaskUFOneView MMAuslan left/best_checkpoints_loss.pth'),
    'FromMaxFlowMask_MMAuslan_right': os.path.join(model_path, 'UFOneView/UFOneView MMAuslan from MaxFlow MaskUFOneView MMAuslan right/best_checkpoints_loss.pth'),
}
...
class FlashAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.scale = qk_scale or head_dim ** -0.5  # Tính scale mặc định nếu không được cung cấp

        # Linear layer để tạo qkv
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = attn_drop  # Lưu xác suất dropout cho attention
    ...
    def forward(self, x):
        ...
        attn_output = flash_attn_qkvpacked_func(
            qkv,
            dropout_p=self.attn_drop if self.training else 0.0,  # Dropout chỉ áp dụng khi training
            softmax_scale=self.scale,
            causal=False  # Uniformer không sử dụng causal attention
        )
    ...
    def get_pretrained_model(self, cfg):
        if cfg.UNIFORMER.PRETRAIN_NAME:
            ...
            for k in checkpoint.keys():
                if k in state_dict_3d and checkpoint[k].shape != state_dict_3d[k].shape:
                    if len(state_dict_3d[k].shape) <= 2:
                        continue
                    time_dim = state_dict_3d[k].shape[2]
                    checkpoint[k] = self.inflate_weight(checkpoint[k], time_dim)

            if 'head.weight' in checkpoint and self.num_classes != checkpoint['head.weight'].shape[0]:
...
def build_uniformer_small(num_classes=174, 
                          pretrained=True, 
                          pretrained_name='uniformer_small_k400_16x4',
                          device='cpu'):
    """
    Xây dựng mô hình Uniformer Small với các tham số mặc định giống config YAML đã cho,
    và (tuỳ chọn) tải trọng số pretrained từ `model_path`.
    
    Args:
        num_classes (int):   Số lớp đầu ra (head) của mô hình.
        pretrained (bool):   Có tải trọng số pretrained hay không.
        pretrained_name(str): Tên checkpoint được dùng để load (nếu có).
        device (str):        Thiết bị 'cpu' hoặc 'cuda'.
    
    Returns:
        model (nn.Module):   Mô hình Uniformer Small.
    """
    class Cfg: pass
    cfg = Cfg()
    ...
    cfg.UNIFORMER.QKV_BIAS = True         # Trong đa số các mô hình, mặc định True
    cfg.UNIFORMER.QKV_SCALE = None        # Nếu không được chỉ rõ, ta để None
    cfg.UNIFORMER.REPRESENTATION_SIZE = None  # Không dùng representation layer
    cfg.UNIFORMER.DROPOUT_RATE = 0.0
    cfg.UNIFORMER.ATTENTION_DROPOUT_RATE = 0.0
    cfg.UNIFORMER.DROP_DEPTH_RATE = 0.1   # stochastic depth
    cfg.UNIFORMER.SPLIT = False           # theo config small
    cfg.UNIFORMER.STD = False             # không dùng conv_3xnxn_std
    if pretrained:
    ...
    # Nếu muốn dùng gradient checkpointing, chỉnh USE_CHECKPOINT = True
    # và chọn checkpoint_num cho từng stage:
    cfg.MODEL.CHECKPOINT_NUM = [0, 0, 0, 0]  # không dùng checkpointing
